...
We take the security of Agent Control seriously. If you believe you have found a
...
When using Agent Control, we recommend following these security best practices:

### Input Validation

- Validate command-line arguments and file paths before processing
- Be cautious when processing data from untrusted sources
- Use the latest version of Agent Control to benefit from security updates

### Dependencies

- Keep Agent Control and its dependencies up to date
...
- Run Agent Control with the minimum required permissions
- Avoid running Agent Control as a privileged user unless necessary
...
Agent Control reads and writes files on the local file system. Users should:
...
Agent Control relies on third-party packages. We:
...
Thank you for helping keep Agent Control and its users safe!

[security-advisories]: https://github.com/demaconsulting/AgentControl/security/advisories
