...
**AgentControl_VersionFlag_Provided_OutputsVersion**: The `--version` flag is passed as
the sole argument; the tool outputs the version string and exits with code 0. This scenario is
tested by `AgentControl_VersionFlag_Provided_OutputsVersion`.

**AgentControl_HelpFlag_Provided_OutputsUsageInformation**: The `--help` flag is passed
...
`AgentControl_HelpFlag_Provided_OutputsUsageInformation`.

**AgentControl_ValidateFlag_Provided_RunsValidation**: The `--validate` flag is passed as
...
`AgentControl_ValidateFlag_Provided_RunsValidation`.

**AgentControl_ValidateWithTrxResults_Requested_GeneratesTrxFile**: The `--validate` flag
...
`AgentControl_ValidateWithTrxResults_Requested_GeneratesTrxFile`.

**AgentControl_ValidateWithXmlResults_Requested_GeneratesJUnitFile**: The `--validate` flag
...
`AgentControl_ValidateWithXmlResults_Requested_GeneratesJUnitFile`.

**AgentControl_SilentFlag_Provided_SuppressesOutput**: The `--version` and `--silent`
...
`AgentControl_SilentFlag_Provided_SuppressesOutput`.

**AgentControl_LogFlag_Provided_WritesOutputToFile**: The `--log <path>` flag is passed
...
`AgentControl_LogFlag_Provided_WritesOutputToFile`.

**AgentControl_UnknownArgument_Provided_ReturnsError**: An unrecognized argument
...
`AgentControl_UnknownArgument_Provided_ReturnsError`.

**AgentControl_ValidateWithDepth_DepthThree_OutputsCorrectHeadingLevel**: The `--validate`
...
`AgentControl_ValidateWithDepth_DepthThree_OutputsCorrectHeadingLevel`.

**AgentControl_NoArguments_Invoked_DisplaysBanner**: The tool is invoked with no
arguments; the combined output contains the tool name and copyright notice and the exit code is
0. This scenario is tested by `AgentControl_NoArguments_Invoked_DisplaysBanner`.

**AgentControl_ResultAlias_LegacyFlag_WritesResultsFile**: The `--validate` flag is
...
`AgentControl_ResultAlias_LegacyFlag_WritesResultsFile`.

**AgentControl_ValidateWithBadExtension_ExtensionInvalid_ReturnsNonZero**: The `--validate`
...
`AgentControl_ValidateWithBadExtension_ExtensionInvalid_ReturnsNonZero`.
